            ...
            async with session.post(url, json=payload, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse JSON with .json(): {json_err}")
                        response_text = await response.text()
                        logger.debug(f"Raw response text (first 500 chars): {response_text[:500]}")
                        try:
                            data = json.loads(response_text.strip())
                            logger.info("Successfully parsed JSON from text after stripping whitespace")
                        except Exception as text_err:
                            logger.error(f"Failed to parse response text as JSON: {text_err}")
                            raise aiohttp.ClientError(f"Invalid JSON response: {text_err}")
                    
                    # Extract content
                    content = data.get("choices", [{}])[0].get("message", {}).get("content", "") or ""
                    logger.info(f"API returned content length: {len(content) if content else 0}")
                    ...
                    if not content:
                        ...
                        if data.get('choices'):
                            ...
                            logger.warning(f"First choice message: {first_choice.get('message', {})}")
                            logger.warning(f"Finish reason: {first_choice.get('finish_reason', 'unknown')}")
                        if data.get('usage'):
                            logger.warning(f"Token usage: {data['usage']}")
                        logger.error("Empty content likely due to: (1) System prompt too long, (2) Model restrictions, or (3) API key issues")
                    else:
...
async def make_request(messages: List[Dict[str, str]], max_tokens: int = 1000, include_sheet_data: bool = True) -> str:
    ...
    """
    # Initialize sheet_data to avoid UnboundLocalError
    sheet_data = ""
    
    logger.debug(f"Global make_request called with {len(messages)} messages, include_sheet_data={include_sheet_data}")
    ...
    # Inject sheet data if available
    if sheet_data:
