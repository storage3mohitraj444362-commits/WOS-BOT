...
class GiftCodeAPI:
    def __init__(self, bot):
        ...
        # Load monitoring channel ID from environment
        monitor_channel_id = os.getenv('GIFTCODE_MONITOR_CHANNEL_ID')
        self.monitor_channel_id = int(monitor_channel_id) if monitor_channel_id else None
        if self.monitor_channel_id:
            self.logger.info(f"[GIFTCODES] 📺 Monitoring channel configured: {self.monitor_channel_id}")
        
        asyncio.create_task(self.start_api_check())

    async def send_monitor_message(self, embed: discord.Embed):
        """Send a monitoring message to the configured Discord channel."""
        ...
        try:
            ...
            if not channel:
                ...
                try:
                    ...
                    self.logger.warning(f"Could not find monitoring channel {self.monitor_channel_id}")
                    return
            
            await channel.send(embed=embed)
        except discord.Forbidden:
            self.logger.warning(f"No permission to send messages to monitoring channel {self.monitor_channel_id}")
        except Exception as e:
            self.logger.error(f"Error sending monitoring message: {e}")
                ...
                try:
                    async with session.get(self.api_url, headers=headers) as response:
                        ...
                        try:
                                        ...
                                        try:
                                            ...
                                            monitor_embed = discord.Embed(
                                                ...
                                                description=(
                                                    ...
                                                    f"**Auto-Redeem:** {'Enabled' if auto_alliances else 'Disabled'} ({len(auto_alliances)} alliances)\n"
                                                ),
                                                color=0x57F287,
                            ...
                            # Send monitoring message if no new codes were found
                            if not new_codes:
                                monitor_embed = discord.Embed(
                                    title="✔️ Gift Code Check Complete",
                                    description=(
                                        f"**Status:** No new codes found\n"
                                        f"**Total Codes in DB:** {len(db_codes)}\n"
                                        f"**Codes from API:** {len(valid_codes)}\n"
                                    ),
                                    color=0x5865F2,
                                    timestamp=datetime.now()
                                )
